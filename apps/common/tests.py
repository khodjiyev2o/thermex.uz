...
from apps.common.models import City, Notification, Region
...
class TestProfile(APITestCase):
    ...
    def test_get_regions_names(self):
        response = self.client.get(reverse("region-list"))
        self.assertEqual(response.status_code, 200)
        assert [region["name"] for region in response.json()] == list(REGION_CHOICES.keys())
    ...
    def test_get_regions_cities_name(self):
        ...
        for region in response.json():
            city = region["name"]
            assert [city["name"] for city in region["cities"]] == list(REGION_CHOICES[city])
